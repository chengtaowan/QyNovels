    ...
    private void initReader() {
        ...
        mAdapter = new ReaderView.Adapter<BookListBean.DataBean.ListBean, BookContentBean.DataBean>() {
            ...
            public String obtainChapterName(BookListBean.DataBean.ListBean listBean) {
                Log.d("back","章节名字:"+ listBean.getName());
            ...
            public String obtainChapterContent(BookContentBean.DataBean dataBean) {
                ...
                Log.d("back","章节内容:"+ dataBean.getContent());
...
//               bookContentPresenter.loadData();
                for(int i=0;i<bookListBean.getData().getList().size();i++){
                    ...
                    Log.d("back","章节ID: " + bookListBean.getData().getList().get(i).getId() + "..." + "章节名字: " + bookListBean.getData().getList().get(i).getName());
                    list.add(itemBean);
                }
                if(list!=null){
//                   testData();
...
//                   mReaderView.invalidateCurrPage();
    ...
    public void testData() {
        int time= com.jdhd.qynovels.readerutil.DeviceInfoUtils.getTime();
        ...
        map.put("id","88382");
        Log.d("back", "testData哪个章节内容id值：88382" );
        String compareTo = com.jdhd.qynovels.readerutil.DeviceInfoUtils.getCompareTo(map);
        String sign= com.jdhd.qynovels.readerutil.DeviceInfoUtils.md5(compareTo);
        map.put("sign",sign);
        if(!token.equals("")){
            RxHttp.postForm(MyApp.Url.baseUrl+"bookContent")
                    ...
                    .subscribe(bookContentBean->{
                        Log.d("back","testData:"+ bookContentBean.getData().getContent()+"--");
//                        bookContent=bookContentBean.getData();
//                        if(count==0){
//                            adapter.notifyDataSetChanged();
//                        }
                        count++;
                        Log.d("back","count:"+ count);
